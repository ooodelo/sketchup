...
- Для аппаратно-ускоренной отрисовки необходима поддержка SketchUp Model Overlay с OpenGL-контекстом (SketchUp 2023.1+). При отсутствии канала плагин автоматически переключится на безопасный режим `view.draw_points`.
...
- В режиме fallback пакетная отправка точек объединена в блоки по 1 000 000 вершин, чтобы минимизировать количество вызовов `view.draw_points`. Цвет и стиль вычисляются один раз на батч, что ускоряет отрисовку при больших облаках.
- Предпочтение «Аппаратная отрисовка overlay» (`prefer_hardware_overlay`) хранится в настройках SketchUp. При ошибке загрузки OpenGL-контекста плагин отключает его и переходит к классическому рендеру. Чтобы повторно активировать ускорение, удалите значение из `SketchUp Preferences` или перезапишите его вручную.
